...
int main(){
	...
	do{
		...
		cin>>b;
		cin.ignore();
		double h=(b-a)/ITERACIONES;
		double suma=0;
		parser::tabSim["x"]=a;
		suma+=parser::eval(expresion);
		if(not parser::error){
			for(int i=1;i<=ITERACIONES;i++){
				if(i%3!=0){
					parser::tabSim["x"]=a+i*h;
					suma+=3.0*parser::eval(expresion);
				}
			}
			for(int i=3;i<=ITERACIONES-3;i+=3){
				parser::tabSim["x"]=a+i*h;
				suma+=2*(parser::eval(expresion));
			}
			cout<<suma*3*h/8<<endl;
		}else{
